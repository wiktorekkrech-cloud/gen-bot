...
const DROP_CHANNEL_ID = "TU_WSTAW_ID_KANAŁU_DO_DROP"; // <-- drop idzie tutaj
...
client.on("messageCreate", async (msg) => {
  ...
  if (msg.content === "!drop") {
    const button = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId("create-drop-btn")
        .setLabel("Utwórz drop 🎁")
    ...
    await msg.channel.send({
      embeds: [
        new EmbedBuilder()
          .setColor(COLOR_ORANGE)
          .setTitle("🎁 Drop")
          .setDescription("Kliknij przycisk poniżej, aby utworzyć drop.")
      ],
      components: [button]
    });
  }
...
client.on("interactionCreate", async (i) => {
  ...
  // ===== DROP PRZYCISK =====
  if (i.isButton() && i.customId === "create-drop-btn") {
    const modal = new ModalBuilder()
